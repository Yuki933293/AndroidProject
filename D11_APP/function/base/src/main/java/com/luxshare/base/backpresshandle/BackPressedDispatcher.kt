 ...
 * 返回键事件分发者
 */
class BackPressedDispatcher constructor(val owner: BackPressedOwner) {
    private val TAG= "BackPressedDispatcher"
    internal val mOnBackPressedCallbacks = ArrayDeque<BackPressedCallback>()

    /**
     * 添加回调
     */
    @MainThread
    fun addCallback(onBackPressedCallback: BackPressedCallback) {
        addCancellableCallback(onBackPressedCallback)
    }

    /**
     * 内部方法，添加可取消的回调。经过此方法，调用者可以通过调用[BackPressedCallback.remove]方法，移除订阅。
     *
     * @param onBackPressedCallback 添加的回调
     * @return 返回一个可以调用[Cancellable.cancel]取消订阅的对象
     */
    @MainThread
    internal fun addCancellableCallback(onBackPressedCallback: BackPressedCallback): Cancellable {
    ...
    fun onBackPressed(): Boolean {
        ...
        while (iterator.hasNext()) {
            ...
            val handleOnBackPressed = callback.handleOnBackPressed(owner)
            Log.i(
                TAG, "onBackPressed: " + callback.isGranted + "--result:" + handleOnBackPressed
            )
            if (callback.isGranted || handleOnBackPressed) {
                return true
            }
        }
        return false
     ...
     */
    private inner class OnBackPressedCancellable internal constructor(private val mOnBackPressedCallback: BackPressedCallback) :
        Cancellable {
    ...
    private inner class LifecycleOnBackPressedCancellable internal constructor(
        ...
        private val mOnBackPressedCallback: BackPressedCallback
